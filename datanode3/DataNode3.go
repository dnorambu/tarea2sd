...
func (s *Server) RequestCompetencia(ctx context.Context, rct *pb.Ricart) (*pb.Okrespondido, error) {
	var err error
	for {
		if s.Estado == "HELD" || s.Estado == "WANTED"{
			//LOG Ocupado por el DN3, siempre que estara en WANTED, independiente del ID del nodo que pide aprobacion, siempre entrara en espera ya que dn3 tiene mayor ID.
		} else {
			//DN3 es un caso especial porque su prioridad es la mas alta en el algoritmo distribuido
...
func (s *Server) crearPropuestaDistribuida() {
	...
	aceptadoPorDn3, err3 := clienteDn1.SendPropuestaDistribuida(context.Background(), propuesta2)
	if err3 != nil {
